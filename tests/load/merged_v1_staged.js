...
export const options = {
  scenarios: {
    ...
    stage1_create_join: {
      ...
      vus: 60, iterations: 1, startTime: "0s", maxDuration: "40s",
      exec: "stage1CreateJoin"
    },
    stage1_verify: {
      executor: "per-vu-iterations",
      vus: 10, iterations: 1, startTime: "30s", maxDuration: "30s",
      exec: "stage1VerifyTeams"
    },
    stage2_strategy: {
      executor: "per-vu-iterations",
      vus: 60, iterations: 1, startTime: "60s", maxDuration: "60s",
      exec: "stage2SubmitStrategy"
    },
    stage2_verify: {
      executor: "per-vu-iterations",
      vus: 10, iterations: 1, startTime: "120s", maxDuration: "30s",
      exec: "stage2VerifyPhase2"
    },
    stage3_vp: {
      executor: "per-vu-iterations",
      vus: 60, iterations: 1, startTime: "150s", maxDuration: "8m",
...
export function stage3VpFeedbackAndConfirm(data) {
  // VP batching: 6 batches of 10 VU, 45s apart (realistic classroom pacing; avoids 60-way DeepSeek pileup)
  sleep(Math.floor(Number(exec.scenario.iterationInTest || 0) / 10) * 45);
  const ctx = scenarioCtx(data);
  const d = ctx.decision;
...
export function stage3FinalizeAndFreeze(data) {
  ...
  const draftRes = jsonPost(`${BASE_URL}/api/team/${encode(team.teamId)}/r1-draft`, {
    memberId: leaderId,
    grid_id: d.gridId,
    architecture: d.architecture
  ...
  const finalizeRes = jsonPost(`${BASE_URL}/api/team/${encode(team.teamId)}/r1-finalize`, {
    memberId: leaderId,
    grid_id: d.gridId,
    architecture: d.architecture,
    vp_text: d.vpText,
