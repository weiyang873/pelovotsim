...
\paragraph{Round 1 的 WTP 调整（v7.1.5 补充）}
在当前实现中，市场锦囊只允许提升解法说服力维度，记调整后的分数为 $E^{adj}$。支付意愿修正不再只依赖 $(G,E^{adj})$，而是同时考虑客户定义是否足够清晰：
\begin{equation}
WTP^{adj}_g = WTP^{ref}_g \cdot \lambda(G)\cdot \lambda(E^{adj})\cdot \rho_{discount}(C),
\label{eq:wtp_adj_round1}
\end{equation}
其中
\begin{equation}
\lambda(s)=0.60+0.10s,\qquad s\in[1,5],
\end{equation}
\begin{equation}
\rho_{discount}(C)=0.75+0.05C,\qquad C\in[1,5].
\end{equation}
直观上，若目标客户仍然定义得过宽，则即便痛点与解法较强，市场也不会给予满额支付意愿溢价；只有当 $C$ 接近 5 时，$\rho_{discount}(C)$ 才接近 1，对溢价不做折扣。
