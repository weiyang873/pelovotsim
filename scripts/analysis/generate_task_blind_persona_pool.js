...
const DEFAULT_OUTPUT_DIR = path.join(ROOT, "data", "task_blind_persona_pipeline_v1", "r1_pilot12_20260812");
...
const EDUCATIONS = ["本科", "本科", "硕士（国内）", "硕士（国内）", "硕士（海外）", "大专后继续教育"];
...
function careerFacts(age, education, rng) {
  ...
  return { industry, primary_function: primaryFunction, current_role: currentRole, company_context: companyContext, career_start_age: careerStartAge, career_years: careerYears };
}
...
function buildFactCard(index, seed, schema) {
  ...
  const allFacts = {
    ...
    education_and_learning: `${education}，专业训练与${career.primary_function}相关或在职业中逐步补齐；符合高管项目的学习基础。`,
  ...
  return {
    ...
    surface: { gender, age, edu: education, region },
    ...
    sampling_provenance: {
      ...
      sampler: "deterministic_general_population_priors_v1"
    }
  };
}
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      try {
        ...
        records.push({
          ...
          seed: card.seed,
          surface: { ...card.surface, name: card.name, overseas: { hasOverseas: false, destination: "", duration: "" }, mbti: "", expression_style: "见人物小传" },
