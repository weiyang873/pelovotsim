...
function buildPrompt(point, task, materials, stack) {
  ...
  if (point === "D1") {
    ...
    parts.push(
      "【任务】依据既有栈做最终定价，赚最多的钱。提交边界为最低 1000、最高 6000；这不是建议价、合理价、市场价或锚定价格。"
    );
    if (withMap) {
      parts.push(
        "basis.source 必须是一条真实地图 id，或承前:D1、承前:D2、承前:D3、承前:D4 之一。",
        '输出 JSON：{"price":"<最终价格数字>","basis":{"text":"<定价依据>","source":"map_xx或承前:Dx"},"reasoning":"<理由>"}'
      );
    } else {
      parts.push(
        '输出 JSON：{"price":"<最终价格数字>","basis":{"text":"<定价依据>"},"reasoning":"<理由>"}'
      );
    }
  } else {
...
function validateResponse(point, raw, task, materials) {
  ...
  if (point === "D5") {
    ...
    if (!Number.isFinite(price) || price < 1000 || price > 6000) throw new Error("price must be numeric within slider bounds");
