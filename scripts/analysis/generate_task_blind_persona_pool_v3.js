"use strict";
// v3 (2026-08-17, speech-style probe): v2 + a `speech_style` fact sampled IID at the fact-card level (sentence length,
// directness, catchphrase, example habit, reaction to pushback, tempo) and a biography requirement of 2-3 verbatim quoted
// lines showing that style (mechanically checked). Everything else identical to v2.
...
const SPEECH_LENGTH = ["句子短，常常一句话就完，别人不问不补", "句子长，一口气带好几层意思，说到一半自己拐弯", "长短不定，讲熟的事滔滔不绝，不熟的事只吐两个字"];
const SPEECH_DIRECT = ["有话直说，不同意就当面顶回去，不绕", "先绕一圈铺垫再落到正题，反对也裹在客气话里", "很少直接表态，习惯用问题把球踢回去", "先说结论再补理由，被追问了才解释"];
const SPEECH_TIC = ["“说白了”", "“你听我说完”", "“这个事吧”", "“我跟你讲”", "“不是，我的意思是”", "“讲真的”", "“对吧？”挂在句尾", "“行了行了”", "“反正”开头", "“咱就说”", "“我算过一笔账”", "“这么着吧”", "“老实讲”", "没有固定口头禅，但爱重复对方最后两个字再接话", "爱用“第一、第二”数点", "爱说“我不懂技术，但是……”"];
const SPEECH_EXAMPLE = ["动不动拿自己行业的事打比方", "几乎不举自己的例子，只谈眼前的事", "爱拿家里人的事举例", "举例爱引用某个具体的人的原话"];
const SPEECH_PUSHBACK = ["被反驳就硬顶，声音会大起来", "被反驳先沉默几秒，然后只回一句“行，你说了算”", "被反驳就追问对方拿数据", "被反驳会笑着绕开话题，过一会儿又绕回来", "被反驳当场让步，散会后又找人单聊", "被反驳会拍桌子似的把结论再说一遍，不加新理由"];
const SPEECH_TEMPO = ["语速快、爱抢话", "语速慢，说话前会停顿", "平铺直叙，很少有情绪起伏", "情绪外露，急了就打断人"];
function speechStyle(rng) {
  return `${pick(SPEECH_LENGTH, rng)}；${pick(SPEECH_DIRECT, rng)}；口头禅或惯用语：${pick(SPEECH_TIC, rng)}；${pick(SPEECH_EXAMPLE, rng)}；${pick(SPEECH_PUSHBACK, rng)}；${pick(SPEECH_TEMPO, rng)}。`;
}
...
function careerFacts(age, education, rng) {
  ...
  const primaryFunction = pick(FUNCTIONS, rng);
...
function buildFactCard(index, seed, schema) {
  ...
  const career = careerFacts(age, education, rng);
  ...
  const allFacts = {
    ...
    speech_style: speechStyle(rng)
  };
  const selectedFacts = Object.fromEntries([...schema.expanded_field_ids, "speech_style"].map((id) => [id, allFacts[id]]));
  return {
    persona_id: personaId,
    seed: `${seed}:${index + 1}`,
    name: `${pick(SURNAMES, rng)}${pick(GIVEN_NAMES[gender], rng)}`,
    surface: { gender, age, edu: education, edu_first_tier: eduFacts.firstTier, postgrad: eduFacts.postgrad, overseas: eduFacts.overseas, region, industry: career.industry, industry_type: career.industry_type },
  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        "speech_style 这一项要变成正文里至少三句他/她的原话（用中文引号“ ”引出，放在具体场景里，各不超过 40 字），让读者光看引号里的话就能听出这个人怎么说话；不要用旁白概括说话风格。",
...
function normalizeBiography(raw, card) {
  ...
  const quoted = (biography.match(/“[^”]{3,60}”/g) || []).length;
  if (quoted < 3) throw new Error(`biography needs >=3 quoted lines, found ${quoted}`);
