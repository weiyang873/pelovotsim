...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--teams") args.teams = Number(argv[++index]);
    ...
    else if (arg === "--team-size") args.configOverrides.team_size = Number(argv[++index]);
    else if (arg === "--max-turns-r1") args.configOverrides.max_turns_r1_discussion = Number(argv[++index]);
    else if (arg === "--max-turns-r2") args.configOverrides.max_turns_r2_per_segment = Number(argv[++index]);
    else if (arg === "--temperature") args.configOverrides.temperature = Number(argv[++index]);
    else if (arg === "--r1-only") args.r1Only = true;
    else throw new Error(`unknown argument: ${arg}`);
  }
  if (!args.arms.length || args.arms.some((arm) => !["simple", "layered", "layered_nomap", "team_layered_nomap", "team_room_roleplay_ui", "team_room_roleplay_stateful_v1", "team_room_roleplay_stateful_review_v1", "team_room_d4_human_pick_v1", "team_room_d4_stateful_pick_v1", "team_room_d4_stateful_d5_nosubmit_v1", "team_room_story_d4_v1", "team_room_story_d4d5_v1", "team_room_story_d4d5_narrator_d5_v1", "team_room_story_r1_d4d5_narrator_v1", "team_room_r1_private_trace_v1", "team_room_r1_story_process_v1", "team_room_r1_reading_story_v1", "team_room_r1_screenplay_v1", "team_room_r1_actor_isolated_v1"].includes(arm))) {
...
async function main() {
  ...
  await Promise.all(tasks.map((task) => limit(async () => {
      console.error(`[team-pilot] start arm=${task.arm} seed=${task.seed}`);
      try {
        ...
        results.push(row);
        console.error(`[team-pilot] done arm=${task.arm} team=${row.team_id} price=${row.price} cards=${row.card_count} profit=${row.profit} r1_only=${row.r1_only}`);
      } catch (error) {
        const failure = {
          arm: task.arm,
          seed: task.seed,
