...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--source-dir") args.sourceDirs.push(resolveRootPath(String(argv[++index] || "").trim()));
    else if (arg === "--source-batch") args.sourceBatches.push(resolveRootPath(String(argv[++index] || "").trim()));
    else if (arg === "--batch") args.batch = String(argv[++index] || "").trim();
    else if (arg === "--output-root") args.outputRoot = resolveRootPath(String(argv[++index] || "").trim());
    else if (arg === "--concurrency") args.concurrency = Number(argv[++index]);
    else if (arg === "--arm") args.arm = String(argv[++index] || "").trim();
    else if (arg === "--pool-path") args.poolPath = resolveRootPath(String(argv[++index] || "").trim());
    else if (arg === "--use-current-config") args.useCurrentConfig = true;
  ...
  if (args.arm && !["legacy", "simple", "layered", "team_room_roleplay_ui"].includes(args.arm)) {
    throw new Error("--arm must be legacy, simple, layered, or team_room_roleplay_ui");
  }
