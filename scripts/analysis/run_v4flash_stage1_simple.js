...
const CONDITIONS = parseConditionsEnv(process.env.V4FLASH_CONDITIONS || "Q,S");
const EXPECTED_POOL_SIZE = parseExpectedPoolSize(process.env.V4FLASH_EXPECTED_POOL_SIZE || "42");
const REP = 1;
...
function parseConditionsEnv(value) {
  const conditions = String(value || "")
    .split(",")
    .map((item) => item.trim().toUpperCase())
    .filter(Boolean);
  const invalid = conditions.filter((item) => !["Q", "S"].includes(item));
  if (!conditions.length || invalid.length) {
    throw new Error(`V4FLASH_CONDITIONS must be a comma-separated list containing only Q/S, got ${value}`);
  }
  return conditions;
}

function parseExpectedPoolSize(value) {
  const parsed = Number(value);
  if (!Number.isInteger(parsed) || parsed < 1) {
    throw new Error(`V4FLASH_EXPECTED_POOL_SIZE must be a positive integer, got ${value}`);
  }
  return parsed;
}
...
async function main() {
  ...
  if (!Array.isArray(pool) || pool.length !== EXPECTED_POOL_SIZE) {
    throw new Error(`persona pool must contain ${EXPECTED_POOL_SIZE} records, got ${Array.isArray(pool) ? pool.length : "not-array"}`);
  }
