...
function writeComparisonMarkdown(filePath, rows, meta) {
  const lines = [
    "# April Summary Simple Replay Comparison",
    "",
    `- run_id: ${meta.runId}`,
    `- persona_mode: simple DeepSeekStudent`,
    `- interview_mode: fixed summary reports`,
...
async function configureDefaultSessionSummary(baseUrl) {
  ...
  if (!res.ok || body.ok === false) {
    throw new Error(`session-config summary failed: ${text}`);
  }
  return body;
}
...
async function main() {
  ...
  const runId = `sim_replay_april_summary_simple_${timestamp}`;
  ...
  await api.health();
  await configureDefaultSessionSummary(baseUrl);
  const results = [];

  for (let teamIndex = 0; teamIndex < REPLAY_STRATEGIES.length; teamIndex += 1) {
    const strategy = REPLAY_STRATEGIES[teamIndex];
    console.log(`[april_summary_simple] ${teamIndex + 1}/${REPLAY_STRATEGIES.length} ${strategy.grid_id} / ${strategy.architecture}`);
  ...
  generateReport(results, reportPath, {
    ...
    logDir,
    exportSummary,
    teacher: { enabled: false, reason: "april replay summary/simple one-off runner" },
    personaMode: "simple",
    interviewMode: "summary"
  });

  const comparisonRows = buildComparisonRows(trackers, logger);
  writeCsv(path.join(logDir, "april_summary_simple_comparison.csv"), comparisonRows);
  writeComparisonMarkdown(path.join(logDir, "april_summary_simple_comparison.md"), comparisonRows, { runId });

  console.log(`\n[april_summary_simple] output: ${logDir}`);
  console.log(`[april_summary_simple] profitable: ${comparisonRows.filter((row) => row.current_profitable === true).length}/${comparisonRows.length}`);
}
