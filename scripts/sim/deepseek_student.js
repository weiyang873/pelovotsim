...
function buildFallbackPhase1(gridId, architecture) {
  ...
  if (parsed.age === "elder") {
    return {
      ...
      how: isDiff
        ? "这款 AI 宠物机器人通过主动靠近、情绪互动和异常提醒，把情感陪伴与轻量守护放进同一个产品体验里"
        : "这款 AI 宠物机器人用更克制的功能组合提供基础陪伴、状态提醒和低学习成本体验，降低机构或家庭导入门槛"
    };
  }
  ...
  if (parsed.age === "child") {
    return {
      ...
      how: isDiff
        ? "这款 AI 宠物机器人通过自然互动、情绪回应和轻任务陪伴，让孩子在关键时段保持投入和安全感"
        : "这款 AI 宠物机器人以可控成本提供基础互动、简单陪伴和可复制体验，帮助家庭或机构先跑通场景"
    };
  }
  ...
  return {
    ...
    how: isDiff
      ? "这款 AI 宠物机器人通过主动迎接、情绪识别和低负担互动，让陪伴感成为一个持续可感知的体验"
      : "这款 AI 宠物机器人用更聚焦的功能组合提供基础陪伴和低维护体验，让购买与使用门槛都更低"
  };
}
...
function buildInterviewFallback(turn, dims) {
  ...
  const list = [
    ...
    `如果是这种有生命感的 AI 宠物机器人介入，您会希望它在哪些地方帮到您，又有哪些边界是不能越过的？`,
...
class DeepSeekStudent {
  ...
  async generatePhase1Choice(gridChoice = null) {
    ...
    const messages = [
      ...
      { role: "user", content: [
        "你是一个 EMBA 学生，正在参加 AI 宠物机器人的中国市场战略模拟。",
        "",
  ...
  async generateInterviewReply(personaMessage, conversationHistory, assignedDims) {
    ...
    const dimText = (Array.isArray(assignedDims) ? assignedDims : [])
      ...
      .join("、");
    const prompt = [
      "你是 EMBA 学生，正在对 AI 宠物机器人的潜在用户做焦点访谈。",
      `你负责的产品维度：${dimText || "综合探索"}`,
      `用户刚说：${JSON.stringify(String(personaMessage || ""))}`,
      "",
      "请用中文继续追问。",
      "前两轮优先聊真实生活场景；第三轮开始可以自然带入 AI 宠物机器人或陪伴机器人。",
      "1-2 句话。"
    ].join("\n");
    const messages = [
  ...
  async generatePriceChoice(priceContext = {}) {
    ...
    const min = Number(priceContext.min || 2000);
    const max = Number(priceContext.max || 6000);
    ...
    const rawPrice = Math.max(min, Math.min(max, Math.round(base * (factor + offset))));
    const steppedPrice = min + Math.round((rawPrice - min) / step) * step;
    const price = Math.max(min, Math.min(max, steppedPrice));
    this.logStudentLLM({
