...
async function speak(member, isLeader, draw, privateProposal, transcript, topic, temperature, arm = "legacy", beat = null) {
  ...
  const stateContext = isStatefulRoomRoleplayArm(arm)
    ...
    : isRoomRoleplayArm(arm) && isTaskBlindNarrativeMember(member)
      ? `\n\n【行为倾向】${behaviorFingerprintForPrompt(member.behavioral_fingerprint)}`
      : "";
...
function taskBlindD4Salience(member) {
  ...
  const options = [
    { score: 0.5 - risk, text: "更容易被安全、可靠、少惹麻烦的卡吸引" },
    { score: 0.5 - promo, text: "更容易被成本低、看起来不出错的卡吸引" },
    { score: promo - 0.5, text: "更容易被能直接讲给客户听的卡吸引" },
    { score: cfc - 0.5, text: "更容易被自己行业里熟悉的系统/运维/渠道逻辑吸引" },
    { score: amb - 0.5, text: "更容易被情感陪伴、互动感、外显体验吸引" }
  ];
...
function taskBlindD4Flaw(member) {
  ...
  const promo = requireFingerprintDim(fp, "regulatory_focus_promotion", id);
  const options = [
    { score: 0.5 - nfc, text: "至少有一张看起来合理的底座卡你会低估、误读或暂时跳过" },
    { score: 0.5 - maxi, text: "至少有一张你会因为名字直观而顺手点，但信心不一定高" },
    { score: cfc - 0.5, text: "至少有一张你会因为成本/研发投入而降档或犹豫" },
    { score: 0.5 - promo, text: "至少有一张你觉得客户未必感知得到，所以不想强推" }
  ];
  options.sort((a, b) => b.score - a.score);
  return options[0].text;
}
