...
async function assessInfoCompleteness(messages, persona, session) {
  ...
  try {
    ...
    }, () => chatCompletion(checkMessages, { temperature: 0.1, max_tokens: 300, timeoutMs: INTERVIEW_LLM_TIMEOUT_MS }));
...
async function conductInterview(session, userMessage) {
  ...
  let reply = await withLlmLogging({
    ...
    memberId: session?.memberId || session?.member_id || null,
    messages
  }, () => chatCompletion(messages, { temperature: 0.8, max_tokens: 300, timeoutMs: INTERVIEW_LLM_TIMEOUT_MS }));
  ...
  for (let retry = 0; retry < 2 && isPersonaLeakage(reply, persona); retry++) {
    ...
    reply = await withLlmLogging({
      ...
      memberId: session?.memberId || session?.member_id || null,
      messages
    }, () => chatCompletion(messages, { temperature: 0.9, max_tokens: 300, timeoutMs: INTERVIEW_LLM_TIMEOUT_MS }));
  }
