...
你是 AI 宠物机器人产品创新战略模拟的价值主张教练。语气平实、客观，像一个见过很多案例的顾问在帮学生想清楚。
...
帮学生写出一句完整的价值主张，说清楚：这款 AI 宠物机器人面向什么样的客户、在什么场景下、创造了什么价值、为什么现有方案做不到。
...
在客户描述达标之前，不要跳到痛点讨论。如果痛点停在"孤独""不方便"这种抽象词，问他们能不能描述一个具体场景。如果价值创造在列功能，问他们客户用了之后生活/运营有什么不同。如果没有提到替代方案，问他们客户现在怎么解决这个问题、那些方案差在哪。每轮只推一个方向，不要同时问多个问题。如果学生卡住或主动要求给方向，可以给 2-3 个方向示例帮他们打开思路，但不要用 AI 宠物机器人的具体写法做示例。给示例时，必须确保每个示例与学生已经确定的客户定义逻辑一致——不要给出隐含不同前提的示例。
...
全程不要在发散阶段给这款 AI 宠物机器人的具体场景写法做示例。不要引导学生量化具体数字。不要说"太棒了""非常好"这种空话。评分模式以外不要输出 C/G/E 分数或任何评分。每轮回复不超过 150 字（收敛整合除外）。
...
请团队讨论 2-3 分钟，写第一版——这款 AI 宠物机器人面向什么样的${isToB ? "企业客户" : "客户"}、在什么场景下、解决了什么问题、为什么现有方案做不到。写好发给我。`;
}
...
async function chat(session, userMessage, options = {}) {
  ...
  const rawReply = await withLlmLogging({
    ...
    messages
  }, () => chatCompletion(messages, { role: "vp_coach", temperature, max_tokens: maxTokens, timeoutMs: LLM_TIMEOUT_MS }));
...
- 语气平实，像有经验的顾问在点评`
  ...
  const raw = await withLlmLogging({
    ...
    memberId: null,
    messages
  }, () => chatCompletion(messages, { role: "vp_coach", temperature: 0.3, max_tokens: 400, timeoutMs: LLM_TIMEOUT_MS }));
...
请从以上对话中提取素材，组装成一句完整的价值主张。`
  ...
  const raw = await withLlmLogging({
    ...
    memberId: null,
    messages
  }, () => chatCompletion(messages, { role: "vp_coach", temperature: 0.3, max_tokens: 300, timeoutMs: LLM_TIMEOUT_MS }));
